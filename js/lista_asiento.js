...
function lista_asiento(verificar) {
    if (verificar == 1) {
    ...
    $.get('lista_asiento/' + fecha1 + '/' + fecha2, function (response) {
        ...
        if(Object.keys(response).length > 0) {
            ...
            for (var i = 0; i < total_filas; i++) {
                ...
                <td>\n\
                <center>\n\
                    <buton class='btn btn-success' onclick='detalle_asiento(" + response[i].id + ")' title='Ver detalle del comprobante'>\n\
                        <i class='fa fa-plus' aria-hidden='true'></i>\n\
                    </buton>\n\
                    <buton class='btn btn-warning' onclick='modificar_asiento(" + response[i].id + ")' title='Modificar comprobante'>\n\
                        <i class='fa fa-pencil' aria-hidden='true'></i>\n\
                    </buton>\n\
                    <buton class='btn btn-danger' onclick='anular_asiento(" + response[i].id + ")' title='Anular comprobante'>\n\
...
function detalle_asiento(id) {
    ...
    $('#loading').css("display", "block");
    $.get('detalle_asiento/' + id, function (response) {
        if(Object.keys(response).length > 0) {
            total_filas = Object.keys(response).length;
...
function anular_asiento(id) {
   $('#id_anular_asiento').val(id);
