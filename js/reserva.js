...
function agregar(){
    ...
    var fila = '<tr id="fila'+cont+'" >\n\
        <td>\n\
          <select class="form-control" name=id_proyecto[] id=id_proyecto'+cont+' onchange=BuscarFase('+cont+') ></select>\n\
        </td>\n\
        <td>\n\
          <select class="form-control" name=id_fase[] id=id_fase'+cont+' onchange=BuscarManzano(this,'+cont+') ></select>\n\
        </td>\n\
        <td>\n\
          <select class="form-control" name=nro_manzano[] id=nro_manzano'+cont+' onchange=BuscarLoteReserva(this,'+cont+') ></select>\n\
        </td>\n\
        <td align=left>\n\
          <select class="form-control" name=nro_lote[] id=nro_lote'+cont+' onchange=Buscar_Lote(this,'+cont+') ></select>\n\
          <input name="id_lote[]" id="id_lote'+cont+'" type="hidden" >\n\
        </td>\n\
        <td>\n\
          Nro. Manzano: <font color="red" id="manzano'+cont+'" size="3"></font><br>\n\
          Precio a Plazo: <font color="red" id="precio'+cont+'" size="3"> </font><br>\n\
          Descuento : <font color="red" id="porcentaje'+cont+'" size="3"> </font><br>\n\
          Precio Contado: <font color="red" id="descuento'+cont+'" size="3"> </font><br>\n\
          Superficie: <font color="red" id="superficie'+cont+'" size="3"> </font><br>\n\
          Categoria: <font color="red" id="categoria'+cont+'" size="3"> </font><br>\n\
        ...
        <td>\n\
          ...
          <select onchange=cargarCuenta(this,'+cont+') style="display:none" name="banco[]" class="form-control" id="banco'+cont+'">\n\
          </select><br>\n\
...
function Eliminar_lista(index){
  ...
  totalesB=0;
  subtotalB= parseFloat($('#subTotalB'+index).val());
  montoBancoB= parseFloat($('#MontoBancoB'+index).val());
  totalesB=parseFloat(subtotalB)+parseFloat(montoBancoB);
  totalB=parseFloat($('#montoTotalB').val());
  restarB=totalB-totalesB;
  $('#montoTotalB').val(restarB);
...
function sumarTotal(input,h){
    if( $(input).attr("id") == "subTotal"+h){
        ...
        $('#subTotalB'+h).val(Total.toFixed(2));
    }
    else{
        MontoBs = parseFloat($('#subTotalB'+h).val());
        Cambio = parseFloat($('#tipocambio').text());
...
function MontoTotalB(id){
  total = 0;
  for (var i = 0; i <= cont; i++) {
    if (!isNaN(parseFloat($("#subTotalB"+i).val()))) {
      total = parseFloat(total) + parseFloat($("#subTotalB"+i).val());          
    }
  }
  $("#montoTotalB").val(parseFloat(total)); 
}
...
function cargarBanco(select,j){
    ...
    if ($(select).val()=="b") {
        ...
        $('#MontoBancoB'+j).css('display','block');
        $('#banco'+j).empty();
        ...
        $('#MontoBanco'+j).val("");
        $('#MontoBancoB'+j).css('display','none');
        $('#MontoBancoB'+j).val("");
        $('#loading').css('display','none');      
    }
}
