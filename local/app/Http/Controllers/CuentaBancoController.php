...
//        $this->middleware('auth', ['only' => 'admin']);
    ...
    public function store(Request $request) {
      ...
      DB::beginTransaction();    
        $cuentabanco=CuentaBanco::create([
          'tipoCuenta' => $request['tipoCuentaG'],
          'moneda' => $request['monedaG'],
          'nroCuenta' => $request['nroCuentaG'],
          'idBanco' => $request['idBancoG']
        ]);
              
        DB::commit();                                               
        return redirect('CuentaBanco/'.$request['idBancoG'])->with('message','GUARDADO CORRECTAMENTE');  
    }catch (Exception $e) {
        DB::rollback();
        return redirect('CuentaBanco/'.$request['idBancoG'])->with("message-error","ERROR INTENTE NUEVAMENTE");     
    }
    ...
    public function update(Request $request) {
        ...
        $id=$request->get("idCuentaM");
        $cuentabanco= CuentaBanco::find($id);
        $cuentabanco->fill([
            'tipoCuenta'=>$request->tipoCuentaM,
            'moneda'=>$request->monedaM,
            'nroCuenta'=>$request->nroCuentaM,
            'idBanco'=>$request->idBancoM
        ]);
        $cuentabanco->save();
        return redirect('CuentaBanco/'.$request['idBancoM'])->with('message','MODIFICADO CORRECTAMENTE');
    }

    public function destroy(Request $request) {

        try {
          $id=$request->get("idCuentaBancoE");
          $cuentabanco = CuentaBanco::find($id);
          $cuentabanco->delete();
          return redirect('CuentaBanco/'.$request['idBancoE'])->with("message","ELIMINADO CORRECTAMENTE");

        }catch (Exception $e) {
            return redirect('CuentaBanco/'.$request['idBancoE'])->with("message-error","NO SE PUDO ELIMINAR EL REGISTRO");      
        } 
    }
