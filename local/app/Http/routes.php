...
Route::get('cargarCuenta/{id}',function($id){
	$cuenta=DB::select('select *from cuentabanco where deleted_at IS NULL and idBanco='.$id);
	return response()->json($cuenta);
});

Route::get('cargarTipoCuenta/{id}',function($id){
	$cuenta=DB::select('select * from cuentabanco where deleted_at IS NULL and id='.$id);
	return response()->json($cuenta);
});
...
Route::get('reprogramar/{idVenta}','ReProgramacionController@reprogramar');
